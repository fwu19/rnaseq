  ...
  ms <- ms %>% 
    ...
    left_join(
      dat$stat %>% 
        ...
        dplyr::select(Sample, contains('FastQC')) %>% 
        mutate(Sample = gsub('_S[0-9]{1,}_L[0-9]{1,}.*', '', Sample)),
      by = 'Sample'
    )
...
figs$uniquely.mapped.reads <- qc_plot(ms, "star.uniquely_mapped", 'Uniquely mapped reads in million', 1e-6)

figs$unique.mapping.pct <- qc_plot(ms, "star.uniquely_mapped_percent", 'Unique mapping percent')
...
```{r}
p <- figs$uniquely.mapped.reads
plotly::ggplotly(p, tooltip = 'text')
```

### Unique mapping percent
```{r}
p <- figs$unique.mapping.pct
plotly::ggplotly(p, tooltip = 'text')
```
...
### Off-target rates
```{r}
p <- figs$off.target.rate
plotly::ggplotly(p, tooltip = 'text')
```

### Zero coverage rates
```{r}
p <- figs$zero.cvg.rate 
plotly::ggplotly(p, tooltip = 'text')
```
