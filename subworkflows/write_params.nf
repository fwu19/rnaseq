...
workflow WRITE_PARAMS {
    ...
    // 1) Parse params_default.config into a Map of defaults
    def cfgFile = new File("${projectDir}/conf/params_default.config")
    def lines   = cfgFile.readLines()

    def inParams    = false
    def BASE_PARAMS = [:]   // Map<String,Object>
    ...
    lines.each { line ->
        ...
        if( inParams && t && !t.startsWith('//') ) {
            ...
            if( parts.size() == 2 ) {
                ...
                BASE_PARAMS[key] = value      // key → default value as String
            }
        }
    }
